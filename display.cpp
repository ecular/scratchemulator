    ...
    case(0x7):
    {
        ...
        for(int y = 0; y < 400; ++y)
            for(int x = 0; x < 640 ; ++x)
            {
                ...
                if(p->video->CRT_Control_Reg_Index == 0x9)
                {
                    char_count_y = y / 4; /*one char is 4 pixels height*/
                    char_addr = vga_start_addr + p->video->Video_Buffer_Address + (char_count_y * p->video->columns + char_count_x) * 2;
                    char_current = p->cpu->ram[char_addr];
                    draw_pixel = p->video->CGA_ascii_table[char_current * 128 + (y % 4) * 8 + (x % 8)]; /*a font in this table ,size is 8(x) * 16 (y). */
                }
                else
                {
