...
Cpu::Cpu(port_handle &ports_operate_arg, Interrupt_Controller_8259a &i8259a_arg, Interval_Timer_8253 &i8253_arg): ports_operate(ports_operate_arg), i8259a(i8259a_arg), i8253(i8253_arg)
{
...
void Cpu::Exec()
{
    ...
    case(0x6C)://INSB (80186)
    {
        if(rep && universal_reg_cx == 0)
            break;
        WriteRam8(*seg_reg_replace << 4 + universal_reg_si, read8_from_port(universal_reg_dx));
    ...
    case(0x6D)://INSD (80186)
    {
        if(rep && universal_reg_cx == 0)
            break;
        WriteRam16(*seg_reg_replace << 4 + universal_reg_si, read8_from_port(universal_reg_dx));
        uint8_t flag_df = (control_reg_flag >> 10) & 0x1;
        if(flag_df) {
    ...
    case(0x6E)://OUSB (80186)
    {
        if(rep && universal_reg_cx == 0)
            break;
        write8_to_port(universal_reg_dx, ReadRam8(*seg_reg_replace << 4 + universal_reg_si));
        uint8_t flag_df = (control_reg_flag >> 10) & 0x1;
        if(flag_df) {
            universal_reg_si = runiversal_reg_si - 1;
            universal_reg_di = runiversal_reg_di - 1;
    ...
    case(0x6F)://OUSW (80186)
    {
        if(rep && universal_reg_cx == 0)
            break;
        write8_to_port(universal_reg_dx, ReadRam16(*seg_reg_replace << 4 + universal_reg_si));
        uint8_t flag_df = (control_reg_flag >> 10) & 0x1;
        if(flag_df) {
            universal_reg_si = runiversal_reg_si - 2;
            universal_reg_di = runiversal_reg_di - 2;
